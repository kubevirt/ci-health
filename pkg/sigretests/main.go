...
func FilterJobsPerSigs(jobs []job, supportedBranches []string) (prSigRetests SigRetests) {
	...
	for _, job := range jobs {
		...
		if checkSIGCIFailure(job) {
			...
			case "performance":
				if job.failure {
					prSigRetests.SigPerformanceFailure += 1
				} else {
					prSigRetests.SigPerformanceSuccess += 1
				}
			}
		}
...
func MapJobNameToSIG(jobName string) string {
	...
	case strings.Contains(jobName, "sig-monitoring"):
		return "monitoring"
	case strings.Contains(jobName, "sig-performance") || strings.Contains(jobName, "performance"):
		return "performance"
	default:
		return ""
	}
